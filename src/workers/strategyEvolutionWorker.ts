...
export class StrategyEvolutionWorker {
  ...
  static initialize() {
    ...
    setInterval(() => {
      this.runPeriodicEvolution().catch(err => {
        console.error("[StrategyEvolutionWorker] Periodic evolution failed:", err);
      });
  ...
  static async evolveStrategy(portfolioId: string) {
    console.log(`[StrategyEvolutionWorker] Evolving strategy for portfolio: ${portfolioId}`);
    try {
      // Phase 7 limit safety
      await StrategyService.enforceStrategyLimit(portfolioId, 10);

      const topStrategies = await StrategyService.getTopStrategies(portfolioId, 1);
      const worstStrategies = await StrategyService.getWorstStrategies(portfolioId, 1);
  ...
  "parameters": {
    ...
    "mean_reversion_factor": 0.8
      ...
      try {
          ...
          const responseText = textResponse.replace(/```json/g, "").replace(/```/g, "").trim();
          ...
          mutatedStrategy = {
              ...
              parameters: {
                  ...best.parameters,
                  confidence_threshold: Math.min(0.9, Math.max(0.4, (best.parameters.confidence_threshold || 0.6) + (Math.random() * 0.1 - 0.05)))
              }
          };
      }

      await StrategyService.mutateStrategy(portfolioId, mutatedStrategy);
      console.log(`[StrategyEvolutionWorker] Mutated strategy ${mutatedStrategy.name} added to profile list.`);

    } catch (err) {
      console.error(`[StrategyEvolutionWorker] Error evolving strategy for params ${portfolioId}`, err);
    }
  }
}
