...
const agentsData = [
  { id: 'Director', name: 'Master Director', role: 'Orchestration', status: 'Active', color: '#00f0ff', confidence: 94, focus: 'Global Strategy Orchestration' },
  { id: 'Quant-v4', name: 'Quant Engine v4', role: 'Data & Signals', status: 'Processing', color: '#a855f7', confidence: 89, focus: 'BTC Volatility Analysis' },
  { id: 'Risk-Guardian', name: 'Risk Guardian', role: 'Constraints', status: 'Monitoring', color: '#facc15', confidence: 99, focus: 'Drawdown Prevention' },
  { id: 'Alpha-Seeker', name: 'Alpha Seeker', role: 'Execution', status: 'Idle', color: '#39ff14', confidence: 82, focus: 'Execution Timing' },
] as const;

export function AIAgentsTab() {
  const [selectedAgentId, setSelectedAgentId] = useState<typeof agentsData[number]['id']>('Director');
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="col-span-1 xl:col-span-4 flex flex-col gap-4">
          ...
          {agentsData.map((agent) => (
             <button 
               ...
               style={{ 
                 borderLeftColor: selectedAgentId === agent.id ? agent.color : undefined, 
                 borderLeftWidth: selectedAgentId === agent.id ? '4px' : '1px',
                 backgroundColor: selectedAgentId === agent.id ? `${agent.color}0a` : undefined,
                 borderColor: selectedAgentId === agent.id ? `${agent.color}40` : undefined,
               }}
             >
                ...
                <div className="w-full mt-1">
                  ...
                  <div className="w-full h-1 bg-[#111] rounded-full overflow-hidden mb-3">
                    <div className="h-full shadow-[0_0_8px_currentColor]" style={{ width: `${agent.confidence}%`, backgroundColor: agent.color, color: agent.color }}></div>
                  </div>
                  <div className="flex justify-between items-start text-[10px] font-mono text-gray-400 bg-[#0a0a0a] p-2 rounded-sm border border-[#1a1a1a]">
                    <span className="uppercase font-bold tracking-widest">Focus:</span>
                    <span className="text-right text-gray-300 w-2/3 leading-tight truncate">{agent.focus}</span>
                  </div>
                </div>
             </button>
          ))}
        ...
        <div className="col-span-1 xl:col-span-8 flex flex-col">
           <div className="bg-[#050505] border border-[#1a1a1a] rounded-sm p-6 relative overflow-hidden flex-1 shadow-2xl flex flex-col h-[400px]">
             ...
             <div className="absolute top-0 right-0 w-96 h-96 blur-[120px] rounded-full pointer-events-none opacity-10 transition-colors duration-1000" style={{ backgroundColor: selectedAgent.color }}></div>
             ...
             <div className="flex justify-between items-center mb-6 border-b border-[#1a1a1a] pb-4 sticky z-10">
               <h3 className="text-gray-300 font-bold text-sm uppercase tracking-widest flex items-center gap-3">
                 <Eye className="w-5 h-5 text-current" style={{ color: selectedAgent.color }} />
               ...
               <div className="flex items-center gap-3 text-[10px] font-bold text-gray-500 uppercase">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="w-16 h-1 bg-[#111] rounded-full overflow-hidden">
                      <div className="h-full transition-all duration-500" style={{ backgroundColor: selectedAgent.color, width: `${selectedAgent.confidence}%` }}></div>
                    </div>
                ...
                <AnimatePresence mode="popLayout">
                  ...
                  >
                    ...
                    <div className="text-gray-300">
                      {selectedAgent.id === 'Director' && 'Synthesizing inputs from Quant-v4 and Risk-Guardian... Market regime identified as Mean-Reverting. Volatility contraction detected.'}
                      {selectedAgent.id === 'Quant-v4' && 'Scanning order books across Binance, Bybit, Coinbase. High frequency iceberg orders detected at $64,150.'}
                      {selectedAgent.id === 'Risk-Guardian' && 'Calculating global drawdown exposure. Current portfolio VAR (Value at Risk) is 0.8%. Within acceptable limits.'}
                      {selectedAgent.id === 'Alpha-Seeker' && 'Awaiting prime entry condition. Limit targets staging...'}
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="text-gray-300">
                      {selectedAgent.id === 'Director' && 'Divergence identified between Spot volume and Perps open interest. Probability of fakeout: High. Directing Alpha-Seeker to hold execution.'}
                      {selectedAgent.id === 'Quant-v4' && 'Generating probability matrix for next 15-minute candle. 68% chance of downward sweep. Feeding to Master Director.'}
                      {selectedAgent.id === 'Risk-Guardian' && 'Simulating stress test for $2,000 flash crash. Liquidation distance is safe. Margin requirement optimal.'}
                      {selectedAgent.id === 'Alpha-Seeker' && 'Adjusting entry bids to match Quant-v4 sweep prediction. New bids placed down to $63,800.'}
                    </div>
                  </motion.div>
                  ...
                  >
                    <div className="text-[10px] flex items-center gap-2 mb-1">
                       <Command className="w-3 h-3" style={{ color: selectedAgent.color }} />
                       <span style={{ color: selectedAgent.color }}>OUTPUT CONCLUSION</span>
                    </div>
                    <div className="text-white font-bold tracking-wide">
                      {selectedAgent.id === 'Director' && 'RESOLUTION: DELAY LONG. Await downward liquidation sweep and reassess.'}
                      {selectedAgent.id === 'Quant-v4' && `SIGNAL VERIFIED: Bearish trap forming. Confidence: ${selectedAgent.confidence}%.`}
                      {selectedAgent.id === 'Risk-Guardian' && 'CLEARANCE GRANTED: All risk parameters green for execution when prompted.'}
                      {selectedAgent.id === 'Alpha-Seeker' && 'ORDERS RESTING: Sniper bounds configured.'}
                    </div>
                  </motion.div>
                </AnimatePresence>
      ...
      <div className="bg-[#050505] border border-[#1a1a1a] rounded-sm p-6 relative">
         ...
         <div className="relative pl-6 space-y-6 before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-[#333] before:to-transparent pt-4">
            ...
            <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#0a0a0a] p-4 rounded border border-[#1a1a1a] shadow">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-gray-200 text-sm">Trade Rejected</div>
                  <time className="text-[10px] text-[#ff4500]">Just now</time>
                </div>
                <div className="text-gray-500 text-xs">Director blocked execution as market conditions no longer support the original thesis.</div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#0a0a0a] p-4 rounded border border-[#1a1a1a] shadow">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-gray-200 text-sm flex flex-row-reverse md:flex-row">Strategy Switched</div>
                  <time className="text-[10px] text-gray-600">3s ago</time>
                </div>
                <div className="text-gray-500 text-xs text-right md:text-left">Quant-v4 shifted to defensive mode, adapting local parameters to defend capital against sudden breakout.</div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#0a0a0a] p-4 rounded border border-[#1a1a1a] shadow">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-gray-200 text-sm">Risk Agent Warning Triggered</div>
                  <time className="text-[10px] text-gray-600">12s ago</time>
                </div>
                <div className="text-gray-500 text-xs">Risk Guardian flagged immediate drawdown risk due to market instability. Margin requirements dynamically increased.</div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#0a0a0a] p-4 rounded border border-[#1a1a1a] shadow">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-gray-200 text-sm flex flex-row-reverse md:flex-row">Volatility Increased</div>
                  <time className="text-[10px] text-gray-600">22s ago</time>
                </div>
                <div className="text-gray-500 text-xs text-right md:text-left">Quant Engine detected sudden volatility expansion across major pairs. Order book spread widening.</div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#0a0a0a] p-4 rounded border border-[#1a1a1a] shadow">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  <div className="font-bold text-gray-200 text-sm">News Spike Detected</div>
                  <time className="text-[10px] text-gray-600">45s ago</time>
                </div>
                <div className="text-gray-500 text-xs">NLP algorithms registered high-impact macroeconomic alert from tier-1 financial sources.</div>
              </div>
            </div>
