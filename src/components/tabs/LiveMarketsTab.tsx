    ...
    <div className="bg-[#050505] border border-[#1a1a1a] rounded-sm p-3 relative overflow-hidden flex flex-col font-mono text-[10px] leading-tight flex-1 min-h-[160px] max-h-[220px]">
      ...
      <h3 className="text-gray-500 font-bold uppercase tracking-widest mb-2 pb-2 flex items-center justify-between border-b border-[#1a1a1a] relative z-10 w-full">
        ...
        <div className="flex gap-1.5 shrink-0">
          ...
          ></span>
          <span
            className="w-1.5 h-1.5 rounded-full bg-[#facc15] animate-pulse"
      ...
      {showDictionary ? (
        ...
        >
          ...
          {patternDictionary.map((pat, i) => (
            <div
              key={i}
              className="flex justify-between items-center bg-[#0a0a0a] p-2 border border-[#111] rounded shadow-sm hover:border-[#222] transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col gap-2 relative z-10">
          <div className="text-gray-600 mb-1">
            Scanning 34 candlestick & structural patterns...
          </div>
          {!detectedPatterns || detectedPatterns.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-4 h-full text-gray-600 text-center gap-2">
              ...
              <span>
                Waiting for structural or candlestick formation signatures...
              </span>
            </div>
          ) : (
            [...detectedPatterns]
              ...
              .map((p, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center bg-[#0a0a0a] p-2 border border-[#111] rounded shadow-sm hover:border-white/10 transition-colors cursor-pointer group"
                >
                  ...
                  <div className="text-right flex flex-col gap-0.5">
                    <span className="text-gray-400 font-bold">
                      {new Date((p.time as number) * 1000)
                        .toISOString()
                        .split("T")[1]
                        .slice(0, -5)}
                    </span>
  ...
  useEffect(() => {
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      if (
        pTime >= l.timeStart &&
        pTime <= l.timeEnd &&
        l.timeEnd > l.timeStart
      ) {
        const ratio = (pTime - l.timeStart) / (l.timeEnd - l.timeStart);
        trendlinePrice =
          l.trendlineStartPrice +
          (l.trendlineEndPrice - l.trendlineStartPrice) * ratio;
      }
    ...
    const detectPatterns = (data: any[]) => {
      ...
      for (let i = 3; i < data.length; i++) {
        ...
        if (bodySize <= candleSize * 0.1 && candleSize > 0) {
          ...
          current.borderColor = "#a855f7";
          current.wickColor = "#a855f7";
        } else if (
          ...
          current.color = "#39ff14";
          current.borderColor = "#00f0ff";
          current.wickColor = "#00f0ff";
        } else if (
          prev.close > prev.open &&
          current.close < current.open &&
          current.open >= prev.close &&
          ...
          current.borderColor = "#ff4500";
          current.wickColor = "#ff4500";
        } else {
          const lowerShadow =
    ...
    <div className="relative w-full h-[407px] ml-0">
        ...
        >
          <div className="flex items-center gap-2 mb-2 pb-2 border-b border-white/5">
            <span
              ...
              style={{
                backgroundColor: activeTooltip.color,
                boxShadow: `0 0 8px ${activeTooltip.color}`,
              }}
            ></span>
            <span className="font-bold font-mono text-[11px] text-white uppercase tracking-widest">
              {activeTooltip.title}
            </span>
          </div>
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-3 pointer-events-none z-20">
        ...
        >
          ...
          <div className="p-3">
            <h4
              className={`font-bold font-mono text-[14px] uppercase mb-3 ${
            ...
            <div className="flex flex-col gap-2 font-mono text-[10px]">
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`font-bold ${aiInsight.risk === "Low" ? "text-[#39ff14]" : aiInsight.risk === "High" ? "text-[#ff4500]" : "text-[#facc15]"}`}
                >
                  {aiInsight.risk}
                </span>
              </div>
        ...
        >
          ...
          <div className="p-3 bg-gradient-to-b from-[#111]/80 to-transparent">
            <div className="flex justify-between items-end mb-3 border-b border-white/5 pb-2">
              ...
              <div className="text-[18px] font-mono font-bold text-[#39ff14] shadow-[0_0_10px_#39ff14] leading-none">
                92%
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 font-mono text-[10px]">
              <div className="flex justify-between items-center">
                <span className="text-[#3b82f6] font-bold px-1.5 py-0.5 bg-[#3b82f6]/10 rounded border border-[#3b82f6]/20">
                  5m
                </span>
                <span className="text-[#39ff14] font-bold pr-1">Bullish</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#a855f7] font-bold px-1.5 py-0.5 bg-[#a855f7]/10 rounded border border-[#a855f7]/20">
                  1H
                </span>
                <span className="text-[#39ff14] font-bold pr-1">Bullish</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#facc15] font-bold px-1.5 py-0.5 bg-[#facc15]/10 rounded border border-[#facc15]/20">
                  4H
                </span>
                <span className="text-[#39ff14] font-bold pr-1">Bullish</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[#00f0ff] font-bold px-1.5 py-0.5 bg-[#00f0ff]/10 rounded border border-[#00f0ff]/20">
                  1D
                </span>
                <span className="text-[#39ff14] font-bold pr-1">Bullish</span>
              </div>
        ...
        <div className="bg-[#050505]/60 backdrop-blur-xl border border-white/10 rounded-xl p-3 shadow-[0_10px_40px_rgba(0,0,0,0.8)] pointer-events-auto self-end flex gap-2">
          ...
          <button
            onClick={() =>
              setToggles((p) => ({ ...p, orderbook: !p.orderbook }))
            }
...
function TradeExecutionPanel({ currentPrice }: { currentPrice?: number }) {
  ...
  const displayPrice =
    orderType === "market" && currentPrice ? currentPrice.toFixed(2) : price;
        ...
        {asks.length > 0
          ? asks.map((ask, i) => {
              ...
              return (
                <div
                  key={`ask-${i}`}
                  className="flex justify-between relative py-0.5 px-1 group cursor-pointer hover:bg-white/5 overflow-hidden"
                >
                  <div
                    className="absolute top-0 right-0 h-full bg-[#ff4500]/15 transition-all duration-300"
                    style={{ width: `${depth}%` }}
                  ></div>
          ...
          : [...Array(7)].map((_, i) => (
              <div
                key={`ask-skel-${i}`}
                className="flex justify-between py-0.5 px-1 text-gray-700 animate-pulse"
              >
                <span>---.-</span>
                <span>-.---</span>
                <span>--.-k</span>
        ...
        {bids.length > 0
          ? bids.map((bid, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="absolute top-0 right-0 h-full bg-[#00f0ff]/10 transition-all duration-300"
                    style={{ width: `${depth}%` }}
                  ></div>
    ...
    >
        ...
        <div className="flex flex-wrap md:flex-nowrap items-center gap-3 md:gap-5 relative z-10 w-full md:w-auto bg-black/40 md:bg-transparent p-3 md:p-0 rounded-lg md:rounded-none border md:border-none border-white/5">
          <div className="flex flex-col items-start md:items-end flex-1 md:flex-auto">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-gray-300 font-mono text-[10px] md:text-xs leading-none">
                WebSockets
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-start md:items-end flex-1 md:flex-auto">
            ...
            <span className="text-[#39ff14] font-mono text-[10px] md:text-xs leading-none drop-shadow-[0_0_8px_rgba(57,255,20,0.5)]">
              2.4ms
            </span>
          </div>
          <div className="hidden md:block h-6 w-[1px] bg-gradient-to-b from-transparent via-white/10 to-transparent"></div>
          <div className="flex flex-col items-start md:items-end flex-1 md:flex-auto">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[#00f0ff] font-mono text-[10px] md:text-xs leading-none drop-shadow-[0_0_8px_rgba(0,240,255,0.5)]">
                Connected
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 relative z-10">
            ...
            <div className="flex justify-between items-center w-full mb-6 relative z-10">
              ...
              <div className="hidden md:flex gap-3">
                <div className="flex items-center gap-2 bg-[#0a0a0a]/50 px-3.5 py-2 rounded-lg border border-white/5 backdrop-blur-md shadow-inner transition-colors hover:border-white/10">
                  ...
                  <span className="text-[#39ff14] text-xs font-bold drop-shadow-[0_0_5px_#39ff14]">
                    Bullish
                  </span>
                </div>
                <button
                  onClick={() =>
                    setToggles((p) => ({ ...p, patterns: !p.patterns }))
                  }
                ...
                <div className="flex items-center gap-2 bg-[#0a0a0a]/50 px-3.5 py-2 rounded-lg border border-white/5 backdrop-blur-md shadow-inner transition-colors hover:border-white/10">
                  ...
                  <span className="text-[#facc15] text-xs font-bold drop-shadow-[0_0_5px_#facc15]">
                    Neutral
                  </span>
                </div>
              </div>
                ...
                {heatmapBlocks.map((row, r) => (
                  <div key={r} className="flex gap-1 flex-1">
                    {row.map((intensity, c) => {
                      ...
                      if (intensity > 0.8)
                        color =
                          "bg-[#ff4500] shadow-[0_0_8px_rgba(255,69,0,0.5)]";
                      else if (intensity > 0.6)
                        color =
                          "bg-[#facc15] shadow-[0_0_8px_rgba(250,204,21,0.3)]";
                      else if (intensity > 0.4)
                        color =
                          "bg-[#00f0ff] shadow-[0_0_8px_rgba(0,240,255,0.3)]";
                      return (
                        <div
                          key={c}
            ...
            <div className="bg-[#020202] border border-white/10 rounded-2xl p-5 flex flex-col h-[200px] shadow-[0_8px_32px_rgba(0,0,0,0.8)] backdrop-blur-xl relative overflow-hidden group">
              ...
              <div className="flex-1 flex flex-col justify-center items-center relative z-10">
                <div className="flex items-center gap-4 w-full px-6">
                  <div className="text-gray-500 font-mono text-[10px] uppercase tracking-widest font-bold">
                    Low
                  </div>
        ...
        <div className="col-span-1 xl:col-span-3 flex flex-col gap-6">
          <TradeExecutionPanel
            currentPrice={dataStream[dataStream.length - 1]}
          />
