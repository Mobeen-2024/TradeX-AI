...
import { useSystemStore, TraceInformation } from '../../store/systemStore';
import { Brain, Activity, Newspaper, Cpu, Play, CheckCircle2, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import Markdown from 'react-markdown';

export const DecisionInspector: React.FC<{ correlationId: string }> = ({ correlationId }) => {
  const { currentTrace, setCurrentTrace, isLoadingTrace, setIsLoadingTrace } = useSystemStore();
  ...
  return (
    <div className="flex flex-col h-full space-y-6 overflow-y-auto no-scrollbar p-6">
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        ...
        <span className="text-xs font-mono text-gray-500">ID: {correlationId.slice(0, 8)}...</span>
      </div>
        ...
        {currentTrace.evaluation && (
          <div className="border border-[#39ff14]/30 bg-[#39ff14]/5 rounded-xl p-4">
             <div className="flex items-center gap-2 mb-3">
               ...
               <h4 className="text-sm font-bold text-white uppercase tracking-wider">Evaluation & Learning</h4>
             </div>
             ...
             {currentTrace.evaluation.metadata && (
                <div className="mt-3 grid grid-cols-3 gap-2 border-t border-white/5 pt-3">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-white">{currentTrace.evaluation.metadata.score}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-500 uppercase">PnL</span>
                    <span className="text-xs text-white">${currentTrace.evaluation.metadata.pnl?.toFixed(2) || 0}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-500 uppercase">Accuracy</span>
                    <span className="text-xs text-white">{currentTrace.evaluation.metadata.accuracy_score}</span>
                  </div>
                </div>
             )}
          </div>
...
const TraceCard = ({ title, icon, color, memory }: { title: string, icon: React.ReactNode, color: string, memory: any }) => {
  return (
    <div className={`border-l-2 ${color} bg-[#0a0a0a] rounded-r-xl p-4 shadow-md`}>
