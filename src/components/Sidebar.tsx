        ...
        <div className="mb-6">
          ...
          <ul className="space-y-0.5">
            <NavItem
              ...
              label="PnL Dashboard"
              active={activeTab === "PnL Dashboard"}
              onClick={() => setActiveTab("PnL Dashboard")}
            />
            <NavItem
              icon={<Activity className="w-3.5 h-3.5" />}
              label="Market Intelligence"
              active={activeTab === "Market Intelligence"}
              onClick={() => setActiveTab("Market Intelligence")}
            />
            <NavItem
              icon={<Cpu className="w-3.5 h-3.5" />}
              label="Multi-Agent Protocol"
              active={activeTab === "Multi-Agent Protocol"}
              onClick={() => setActiveTab("Multi-Agent Protocol")}
            />
            <NavItem
              icon={<Zap className="w-3.5 h-3.5" />}
              label="Autonomous Execution"
              active={activeTab === "Autonomous Execution"}
              onClick={() => setActiveTab("Autonomous Execution")}
            />
          </ul>
        </div>
        ...
        <div className="mb-6">
          ...
          <ul className="space-y-0.5">
            <NavItem
              ...
              active={activeTab === "Strategy Engine"}
              onClick={() => setActiveTab("Strategy Engine")}
            />
            <NavItem
              icon={<Database className="w-3.5 h-3.5" />}
              label="Agent Memory"
              active={activeTab === "Agent Memory"}
              onClick={() => setActiveTab("Agent Memory")}
            />
            <NavItem
              icon={<LineChart className="w-3.5 h-3.5" />}
              label="Technical Engine"
              active={activeTab === "Technical Engine"}
              onClick={() => setActiveTab("Technical Engine")}
            />
            <NavItem
              icon={<TestTube className="w-3.5 h-3.5" />}
              label="Backtesting Engine"
              active={activeTab === "Backtesting Engine"}
              onClick={() => setActiveTab("Backtesting Engine")}
            />
          </ul>
        </div>
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-0.5">
            <NavItem
              ...
              active={activeTab === "System Telemetry"}
              onClick={() => setActiveTab("System Telemetry")}
            />
            <NavItem
              icon={<Shield className="w-3.5 h-3.5" />}
              label="Audit & Governance"
              active={activeTab === "Audit & Governance"}
...
function NavItem({
  ...
  return (
    ...
    >
      <button
        onClick={onClick}
        className={`w-full flex items-center justify-between px-3 py-2 rounded transition-colors duration-200 text-[12px] group relative ${active ? "text-[#00f0ff]" : "text-gray-400 hover:text-gray-200"} overflow-hidden`}
      >
        {active && (
          <motion.div
        ...
        <div className="flex items-center gap-3 relative z-10 whitespace-nowrap">
          ...
          <span className="font-mono tracking-wide uppercase opacity-0 group-hover/sidebar:opacity-100 transition-opacity duration-300">
            {label}
          </span>
        </div>
        {badge && (
