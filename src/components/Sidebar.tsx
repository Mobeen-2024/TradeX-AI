...
export function Sidebar({ activeTab, setActiveTab }: { activeTab: TabType, setActiveTab: (tab: TabType) => void }) {
  return (
    <aside className="w-[280px] flex-shrink-0 bg-[#020202] border-r border-[#1a1a1a] flex flex-col h-full relative z-10 transition-all duration-300">
      <div className="p-4 overflow-y-auto no-scrollbar flex-1 pb-32 w-full block scroll-smooth">
        {/* Core Identity */}
        <div className="flex items-center justify-between mb-8 px-3 mt-4">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#00f0ff]/10 border border-[#00f0ff]/30 flex items-center justify-center shadow-[0_0_15px_rgba(0,240,255,0.2)]">
        ...
        <button className="w-full relative group mb-8">
          ...
          <div className="relative flex items-center justify-between bg-[#050505] hover:bg-[#0a0a0a] border border-[#1a1a1a] border-l-[#00f0ff] border-l-[2px] text-sm text-gray-200 py-3 px-4 rounded-lg transition-colors">
            <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-widest">
              ...
              <span className="font-bold text-white/90">Initialize Agent</span>
            </div>
          </div>
        </button>
...
function NavItem({ icon, label, badge, active, onClick }: { icon: React.ReactNode, label: string, badge?: string, active?: boolean, onClick?: () => void }) {
  ...
  return (
    ...
    >
      <button onClick={onClick} className={`w-full flex items-center justify-between px-3 py-2 rounded transition-colors duration-200 text-[12px] group relative ${active ? 'text-[#00f0ff]' : 'text-gray-400 hover:text-gray-200'}`}>
        ...
        <div className="flex items-center gap-3 relative z-10">
          <span className={`transition-colors duration-300 ${active ? 'text-[#00f0ff]' : 'opacity-50 group-hover:opacity-80'}`}>
            {icon}
          </span>
          <span className="font-mono tracking-wide uppercase">{label}</span>
        </div>
        {badge && (
          <span className="relative z-10 text-[9px] uppercase tracking-widest font-bold bg-[#39ff14]/10 text-[#39ff14] border border-[#39ff14]/30 px-1.5 py-0.5 rounded-sm flex items-center shadow-[0_0_8px_rgba(57,255,20,0.15)]">
            {badge}
          </span>
        )}
      </button>
