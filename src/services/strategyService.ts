...
export class StrategyService {
  static async getActive(portfolioId: string) {
    const strategy =
      await StrategyProfileRepository.getActiveStrategy(portfolioId);
    if (!strategy) {
      return {
        parameters: {
  ...
  static async enforceStrategyLimit(
    portfolioId: string,
    maxLimit: number = 10,
  ) {
    const pool = require("../db/connection").getPool();
    const countRes = await pool.query(
      "SELECT count(*) as total FROM strategy_profiles WHERE portfolio_id = $1",
      [portfolioId],
    );
