...
class ExecutionSimulator {
    static async simulateExecution(intendedPrice: number, action: 'BUY' | 'SELL'): Promise<{ executedPrice: number, slippageBps: number, latencyMs: number, spreadPaid: number }> {
        const latencyMs = Math.floor(Math.random() * 250) + 50;
        await new Promise(r => setTimeout(r, latencyMs));

        const spreadBps = Math.random() * 2;
        const spreadMultiplier = action === 'BUY' ? (1 + spreadBps/10000) : (1 - spreadBps/10000);
        
        const slippageBps = Math.random() * 5;
        const slippageMultiplier = action === 'BUY' ? (1 + slippageBps/10000) : (1 - slippageBps/10000);
...
export class ExecutionService {
    ...
    static async placeOrder(request: OrderRequest): Promise<{ orderId: string, price: number }> {
        ...
        try {
            ...
            if (request.correlationId) {
                const existing = await client.query(`SELECT id, status, average_fill_price FROM orders WHERE correlation_id = $1`, [request.correlationId]);
                if (existing.rows.length > 0) {
                    if (existing.rows[0].status === 'FILLED' || existing.rows[0].status === 'PENDING' || existing.rows[0].status === 'NEW') {
                        return { orderId: existing.rows[0].id, price: Number(existing.rows[0].average_fill_price || 0) };
                    }
                }
            }

            // SAFETY LAYER: Global Kill Switch & Circuit Breaker
            const systemRes = await client.query(`SELECT is_trading_enabled, circuit_breaker_active, exchange_api_failures FROM global_system_controls ORDER BY id ASC LIMIT 1`);
            if (systemRes.rows.length > 0) {
                const sys = systemRes.rows[0];
                if (!sys.is_trading_enabled || sys.circuit_breaker_active) {
                    throw new Error("Execution rejected: Global trading is disabled or circuit breaker is active.");
                }
            }

            // Portfolio Hard Risk Constraints
            const portfolioRes = await client.query(`SELECT user_id, is_trading_enabled, max_position_size, max_loss FROM portfolios WHERE id = $1`, [request.portfolioId]);
            ...
            if (!portfolio.is_trading_enabled) {
                throw new Error("Execution rejected: Trading is disabled for this portfolio.");
            }

            // Verify Loss Streaks via RiskControlService
            const streakMultiplier = await RiskControlService.checkLossStreak(request.portfolioId);
            ...
            if (request.correlationId) {
                 const coordMemory = await MemoryService.getByCorrelation(request.correlationId, "Coordinator");
                 if (coordMemory && coordMemory.metadata && typeof coordMemory.metadata.confidence_score === 'number') {
                      confidenceScore = coordMemory.metadata.confidence_score;
                 }
                 const quantMemory = await MemoryService.getByCorrelation(request.correlationId, "QuantAgent");
                 if (quantMemory) {
                     marketRegime = quantMemory.market_regime || "UNKNOWN";
                     volatilityLevel = (quantMemory.metadata && quantMemory.metadata.volatility_level) || "NORMAL";
                 }
            }

            // Verify Max Loss Limit vs Current PNL
            const positionsRes = await client.query(`SELECT * FROM positions WHERE portfolio_id = $1`, [request.portfolioId]);
            ...
            for (const pos of positionsRes.rows) {
                 ...
                 totalUnrealizedPnl += (currPrice - Number(pos.avg_entry_price)) * Number(pos.size);
            }
            if (Number(portfolio.max_loss) > 0 && (totalRealizedPnl + totalUnrealizedPnl) <= -Math.abs(Number(portfolio.max_loss))) {
                 throw new Error(`Execution rejected: Max loss limit (${portfolio.max_loss}) breached.`);
            }
            ...
            if (recentTicks.rows.length >= 10) {
                ...
                const spreadPercentage = avgPrice > 0 ? (maxPrice - minPrice) / avgPrice : 0;
            ...
            // Apply all position sizing edges: base size * streak edge * vol curve * confidence probability
            request.size = parseFloat((originalSize * streakMultiplier * volatilityMultiplier * confidenceScore).toFixed(6));
            
            // Phase 8: Capital Allocation scaling
            if (request.strategyId) {
                const { CapitalAllocationService } = require('./capitalAllocationService');
                const allocs = await CapitalAllocationService.getContextualAllocations(request.portfolioId, marketRegime, volatilityLevel);
                const alloc = allocs.find((a: any) => a.strategy_id === request.strategyId);
                const allocationMultiplier = alloc ? Number(alloc.allocation_percentage) : 1;
            ...
            console.log(`[ExecutionService] Original request volume: ${request.size} @ ~${currentPrice} = ${expectedNotional} USDT. Confidence: ${confidenceScore}`);
            ...
            if (!isBacktestMode) {
                ...
                if (expectedNotional > MAX_NOTIONAL) {
                   ...
                   console.log(`[ExecutionService] Capping size to max notional ($50): ${finalSize}`);
                   ...
                   console.log(`[ExecutionService] Bumping size to min notional ($15): ${finalSize}`);
                }
            }
            ...
            console.log(`[ExecutionService] Executing order: ${request.action} ${request.size} ${request.assetId}`);
            ...
            console.log(`[ExecutionService] Inserted internal PENDING order: ${internalOrderId}`);
            ...
            if (!isBacktestMode) {
                ...
                try {
                    ...
                    console.log(`[ExecutionService] Exchange response status: ${executionStatus}, target ID: ${exchangeOrderId}`);

                    
                    if (orderRes.status === 'FILLED' || orderRes.status === 'PARTIALLY_FILLED') {
                        let totalCost = 0;
                        let totalQty = 0;
                        if (orderRes.fills && orderRes.fills.length > 0) {
                            ...
                            executedPrice = parseFloat(orderRes.cummulativeQuoteQty) / parseFloat(orderRes.executedQty);
                            filledQuantity = parseFloat(orderRes.executedQty);
                        }
                    }

                    // Reset API failures on success
                    await client.query(`UPDATE global_system_controls SET exchange_api_failures = 0`);
                } catch (err) {
                    ...
                    console.error(`[ExecutionService] Binance API Error. Failures count: ${failures}`);

                    if (failures >= 3) {
                        await client.query(`UPDATE global_system_controls SET circuit_breaker_active = true, is_trading_enabled = false`);
                        console.error(`[ExecutionService] CIRCUIT BREAKER TRIGGERED! Trading halted due to 3 consecutive exchange API failures.`);
                    }

                    await client.query(`UPDATE orders SET status = 'CANCELED' WHERE id = $1`, [internalOrderId]);
                    throw new Error(`Real exchange execution failed: ${(err as any).message}`);
                }
            } else {
                // Backtest mode Simulation
                const sim = await ExecutionSimulator.simulateExecution(currentPrice, request.action);
            ...
            await client.query(
                `UPDATE orders 
                 ...
                 WHERE id = $5`,
                [executionStatus, exchangeOrderId, executedPrice, filledQuantity, internalOrderId]
            );

            // If order is fully or partially filled, we apply it locally
            // (If it's just 'NEW', the SyncWorker will process the fills later)
            if (executionStatus === 'FILLED' || executionStatus === 'PARTIALLY_FILLED') {
                await ExecutionService.processFills(
                    client, request, internalOrderId, executedPrice, filledQuantity, portfolio, positionsRes.rows
                );
            }
    ...
    private static async processFills(
        client: any, request: OrderRequest, orderId: string, price: number, filledSize: number, portfolio: any, positions: any[]
    ) {
        ...
        if (balanceResult.rows.length === 0) {
            await client.query(`INSERT INTO balances (portfolio_id, cash_balance) VALUES ($1, $2)`, [request.portfolioId, cash]);
        }
        ...
        if (request.action === 'BUY') {
            ...
            await client.query(`UPDATE orders SET trade_id = $1 WHERE id = $2`, [newTradeId, orderId]);
        }
        ...
        if (request.action === 'SELL') {
            ...
            if (pos) {
                ...
                const realizedPnl = PnlService.calculateRealizedPnlOnSell(entryPrice, price, filledSize);
            ...
            if (openTradeResult.rows.length > 0) {
                ...
                const tradeRealizedPnl = PnlService.calculateRealizedPnlOnSell(Number(tradeToClose.entry_price), price, filledSize);
                
                await client.query(
                    `UPDATE trades SET status = 'CLOSED', exit_price = $1, pnl = $2, closed_at = NOW() WHERE id = $3`,
                    [price, tradeRealizedPnl, tradeToClose.id]
                );
                await client.query(`UPDATE orders SET trade_id = $1 WHERE id = $2`, [tradeToClose.id, orderId]);
                closedTradeId = tradeToClose.id;
            }
        }

        if (closedTradeId && portfolio.user_id) {
            EvaluationService.evaluateTrade(closedTradeId, portfolio.user_id, request.portfolioId, request.correlationId).catch(err => {
        ...
        if (simMetrics && targetTradeId) {
            await client.query(
                `INSERT INTO execution_metrics (trade_id, intended_price, executed_price, slippage_bps, latency_ms, spread_paid)
                 VALUES ($1, $2, $3, $4, $5, $6)`,
                [targetTradeId, simMetrics.intendedPrice, simMetrics.executedPrice, simMetrics.slippageBps, simMetrics.latencyMs, simMetrics.spreadPaid]
            );
        }
    }
    ...
    static async syncOpenOrders() {
        ...
        try {
            ...
            if (pendingOrders.rows.length > 0) {
                 console.log(`[ExecutionService] Found ${pendingOrders.rows.length} pending exchange orders.`);
            }

            for (const order of pendingOrders.rows) {
                try {
                    const statusRes = await this.binanceClient.getOrderStatus(order.asset_id, order.exchange_order_id);
                    ...
                    if (statusRes.status !== order.status) {
                        ...
                        if (['FILLED', 'PARTIALLY_FILLED', 'CANCELED', 'REJECTED'].includes(statusRes.status)) {
                    ...
                    console.error(`[ExecutionService] Failed to sync order ${order.id}:`, err);
                }
            }
            return pendingOrders.rows;
        } finally {
