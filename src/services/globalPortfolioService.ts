...
export class GlobalPortfolioService {
    static async getGlobalState(): Promise<GlobalCapitalState> {
        ...
        try {
            ...
            let totalCash = balances.rows.reduce((sum, r) => sum + Number(r.cash_balance), 0);
            
            // 2. Get all active positions
            const positions = await client.query(`SELECT asset_id, size, avg_entry_price, side FROM (
            ...
            for (const asset of assets) {
                const tick = await client.query(`SELECT price FROM market_ticks WHERE symbol = $1 ORDER BY timestamp DESC LIMIT 1`, [asset]);
            ...
            for (const pos of positions.rows) {
                const currPrice = currentPrices[pos.asset_id] || Number(pos.avg_entry_price);
                ...
                if (pos.side === 'LONG' || pos.side === 'BUY') {
                    ...
                    netExposure -= posValue;
                    unrealizedPnl += (Number(pos.avg_entry_price) - currPrice) * Number(pos.size);
                }
            }
            ...
            const allTrades = await client.query(`SELECT pnl FROM trades WHERE status = 'CLOSED' ORDER BY closed_at ASC`);
            ...
            const globalDrawdown = peakValue > 0 ? Math.max(0, (peakValue - trueCurrentValue) / peakValue) : 0;
            
            // LAYER 3: CROSS-PORTFOLIO RISK
            let volSpikeCount = 0;
            for (const asset of assets) {
                const recentTicks = await client.query(`SELECT price FROM market_ticks WHERE symbol = $1 ORDER BY timestamp DESC LIMIT 20`, [asset]);
            ...
            const correlationRisk = assets.length > 1 && volSpikeCount >= assets.length / 2 ? 0.8 : 0.0;
    ...
    static async getPortfolioWeight(portfolioId: string): Promise<number> {
        ...
        try {
            ...
            if (state.globalDrawdown > 0.10) {
                globalPenalty += (state.globalDrawdown - 0.10) * 5; // e.g. 0.15 DD -> 0.25 penalty
            }
            ...
            if (state.totalCapital > 0 && state.totalExposure > state.totalCapital * 3) {
            ...
            if (state.totalExposure > 0 && (state.netExposure / state.totalExposure) > 0.7) {
            ...
            // LAYER 2: CAPITAL PARTITIONING (Dynamic rebalancing based on portfolio perf)
            let portfolioWeightBase = 1.0;
            const pnlRes = await client.query(`SELECT sum(pnl) as tpnl FROM trades WHERE portfolio_id = $1 AND status = 'CLOSED'`, [portfolioId]);
            const pPnl = Number(pnlRes.rows[0]?.tpnl || 0);
            
            if (pPnl > 5000) {
                portfolioWeightBase += 0.1; // Boost allocations to winning ports
            } else if (pPnl < -5000) {
                portfolioWeightBase -= 0.2; // Penalize losing ports
            }
            
            // Combine weights and penalties
            let finalWeight = portfolioWeightBase - globalPenalty;
